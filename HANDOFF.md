...
| **① Timing** | Speed (× — reveal pace, ÷ into `DUR_BASE`) · **Opacity/scale speed** (Connect/Disconnect: link both tracks' Speed to this one) · Start delay · **Sweep easing** (cubic-bézier) · **Opacity/scale easing** (Connect/Disconnect: link both tracks' easing to this curve) |
| **② Reveal pattern** | **Pattern** (Sweep / Spotty) — sets the reveal *order*. Sweep: Direction (0–360°) · Stagger · Curve bow. Spotty: **Seed count** (few large ↔ many small patches) · **Spread speed** (× patch growth) · **Edge softness** (soft matte band width) · **Blob** (fBm warp: 0 = circles → organic blobs) · **Blob scale** (noise zoom) · **⟳ Re-roll seeds** |
